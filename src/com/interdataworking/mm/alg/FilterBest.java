...
/**
 * Filters the best matches efficiently without sorting the pairs. If unique flag is set, matches with more than one candidate are discarded. (a, c, 1), (a, d, 1), (b, c, 1), (b, d, 1)
 * 
...
public class FilterBest implements UntypedGateway {
	...
	public void pass(Map<RDFNode, int[]> map, RDFNode n, MapPair p, int i, int[] penalty) {

		if (p.sim_ <= 0) { // trash this pair
			penalty[i]++;
			return;
		}
		...
		if (cand == null) {
			...
			double diff = pairs[cand[0]].sim_ - p.sim_;
	...
	public List<MapPair> getFilterBest(Collection<? extends MapPair> arr, boolean unique) {
		...
		Map<RDFNode, int[]> map1 = new HashMap<RDFNode, int[]>(); // RDFNode -> int[] of positions
		...
		Map<RDFNode, int[]> map2 = new HashMap<RDFNode, int[]>(); // map1 still needed if unique requested
	...
	void penalizeNonUnique(Map<RDFNode, int[]> map, int[] penalty) {

		for (Iterator<int[]> it = map.values().iterator(); it.hasNext();) {
	...
	public List<Model> execute(List<Model> input) throws ModelException {
		...
		Model src = input.get(0);

		List<MapPair> l1 = MapPair.toMapPairs(src);
		List<MapPair> best = getFilterBest(l1, true);
	...
	public static void main(String[] args) throws Exception {
		...
		List<MapPair> l = new ArrayList<MapPair>();
		...
		Collection<MapPair> result = new FilterBest().getFilterBest(l, true);
