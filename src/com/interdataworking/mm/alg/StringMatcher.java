...
public class StringMatcher implements UntypedGateway {
	...
	HashSet<MapPair> initialMatch /* pairs that used only for init */, reinforceMatch_ /* pairs that are used for reinforcing */;

	HashMap<String, List<String>> strCompMap = new HashMap<String, List<String>>(); // string -> List

	Map<String, IntHolder> wordFreqM1, wordFreqM2;

	Map<String, MapPair> strSim; // string -> MapPair, given string similarity
	...
	public double computeStringSimilarity(String s1, String s2) {
		...
		List<String> l1 = getWords(s1);
		List<String> l2 = getWords(s2);

		return computeStringSimilarity(l1, l2);
	}

	public static double computeStringSimilarity(List<String> l1, List<String> l2) {

		return computeStringSimilarity(l1, l2, null, null);
	}

	public static double computeStringSimilarity(List<String> l1, List<String> l2, Map<String, IntHolder> freq1, Map<String, IntHolder> freq2) {
		...
		for (int i = 0; i < l1.size(); i++) {
			...
			for (int j = 0; j < l2.size(); j++) {
				...
				if (freq1 != null) {
					...
					if (f <= 0)
						throw new InternalError("Cannot happen: no freq1 for " + t1);
					else if (f > 1)
						f *= FREQ_PENALTY;
					rs /= f;
				}
				...
				if (freq2 != null) {
					...
					if (f <= 0)
						throw new InternalError("Cannot happen: no freq2 for " + t2);
					else if (f > 1)
						f *= FREQ_PENALTY;
					rs /= f * FREQ_PENALTY;
				}
	...
	// use map as cache
	public static List<String> getWords(String s, Map<String, List<String>> m) {
	...
	public static List<String> getWords(String s) {
		...
		Iterator<String> it = new StringComponentIterator(s);
	...
	public List<Model> execute(List<Model> input) throws ModelException {
		...
		Model m1 = input.get(0);
		Model m2 = input.get(1);

		if (input.size() >= 3) {
			Model strm = input.get(2);
		...
		Collection<MapPair> result = initialGuess(m1, m2);

		switch (DISTORT) {

		case RANDOMIZE:
			MapPair.randomizeMap(result, RANDOMIZE_PERCENT);
			break;
		case AVERAGIZE:
			MapPair.averagizeMap(result);
			break;
		...
		MapPair[] arg = new MapPair[0];
		Model map = MapPair.asModel(m1.create(), new ArrayList<MapPair>(result).toArray(arg));
		l.add(map);
		return l;
	}
	...
	Map<String, IntHolder> getWordFreq(Model m) throws ModelException {

		Map<String, IntHolder> wordFreq = new HashMap<String, IntHolder>();

		for (Enumeration<RDFNode> en1 = RDFUtil.getNodes(m).elements(); en1.hasMoreElements();) {

			RDFNode n1 = en1.nextElement();
			...
			List<String> l = StringMatcher.getWords(s, strCompMap);

			// System.err.println("getWordFreq3: " + l);

			for (int i = 0; i < l.size(); i++)
				incFreqOf(wordFreq, l.get(i));
	...
	static int getFreq(Map<String, IntHolder> wordFreq, String word) {

		IntHolder i = wordFreq.get(word);
		return i == null ? 0 : i.value;
	}

	static void incFreqOf(Map<String, IntHolder> wordFreq, String word) {
	...
	@SuppressWarnings("unchecked")
	public Collection<MapPair> initialGuess(Model m1, Model m2) throws ModelException {

		HashSet<MapPair> reinforceMatch = new HashSet<MapPair>();
		...
		for (Enumeration<RDFNode> en1 = RDFUtil.getNodes(m1).elements(); en1.hasMoreElements();) {

			RDFNode n1 = en1.nextElement();
			if (n1 instanceof Statement)
				continue;

			for (Enumeration<RDFNode> en2 = RDFUtil.getNodes(m2).elements(); en2.hasMoreElements();) {

				RDFNode n2 = en2.nextElement();
	...
	boolean isOpaqueResource(RDFNode n1) throws ModelException {
		...
		if (n1 instanceof Resource) {
			...
			if ((!TRY_OPAQUE_RESOURCES && ns == null) || (ns != null && ns.startsWith("urn:rdf:"))) // StringMatcher.opaque(ns)))
				return true;
		}
		return false;
	}
	...
	double computeNodeSimilarity(RDFNode n1, RDFNode n2) throws ModelException {
		...
		if (s1 != null && s2 != null) {
			...
			List<String> l1 = StringMatcher.getWords(s1, strCompMap);
			List<String> l2 = StringMatcher.getWords(s2, strCompMap);

			sim = computeStringSimilarity(l1, l2, wordFreqM1, wordFreqM2);
		}

		// use a damping factor when matching a literal against a resource
		if (sim > 0 && (n1 instanceof Literal && n2 instanceof Resource) || (n2 instanceof Literal && n1 instanceof Resource))
			sim /= 2;
